...
use crate::write::comp_writer::CompWriter;
...
impl<'w, W: Write + Seek + 'w> Write for File<'w, W> {
    fn write(&mut self, buf: &[u8]) -> IoResult<usize> {
        ...
        self.raw_hasher.update(&buf[0..written]);
        Ok(written)
    }
...
impl<'w, W: Write + Seek + 'w> Drop for File<'w, W> {
    fn drop(&mut self) {
        ...
        let raw_checksum: [u8; 32] = *self.raw_hasher.finalize().as_bytes();
        ...
        bincode_write(&mut self.comp_writer, &file_header).unwrap();
        ...
        println!(
            "Written file header from byte #{} to #{}.",
            file_header_begin, file_header_end
        );
        println!("RAW SIZE OF THIS FILE: {}", self.raw_size);
        self.directory
            .set_file(&self.filename, file_header_begin..file_header_end);
    }
}
